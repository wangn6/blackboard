...
default_start_threads = 50
default_maximum_threads = 1000
default_increase_steps = 10
default_durations = 60 * 2
default_warmup_period = 10
default_error_rate_limit = 0.01
default_response_time_limit = 1000
test_type = 'stress_test'

# Get the input parameters firstly
start_num_threads = (ENV["Start_Num_Threads"] == "Default" || ENV["Start_Num_Threads"] == nil) ? default_start_threads : ENV["Start_Num_Threads"].to_i
increase_step = (ENV["Increase_Step"] == "Default" || ENV["Increase_Step"] == nil) ? default_increase_steps : ENV["Increase_Step"].to_i
target_num_threads = (ENV["Target_Num_Threads"] == "Default" || ENV["Target_Num_Threads"] == nil) ? default_maximum_threads : ENV["Target_Num_Threads"].to_i
test_plan_template = (ENV["Test_Plan"] == nil || ENV["Test_Plan"] == "") ? nil : ENV["Test_Plan"]
#loops = ENV["Loops"] == "Default" ? nil : ENV["Loops"].to_i
duration = (ENV["Duration"] == "Default" || ENV["Duration"] == nil) ? default_durations : ENV["Duration"]
warmup_period = (ENV["Warmup_Period"] == "Default" || ENV["Warmup_Period"] == nil) ? default_warmup_period : ENV["Warmup_Period"]
target_maximum_error_rate = ENV["Error_Rate"] == "Default" ? default_error_rate_limit : ENV["Error_Rate"].to_f#1%
target_avg_response_time = ENV["Response_Time"] == "Default" ? default_response_time_limit : ENV["Response_Time"].to_f#1s
...
testplanTemplates.each do |testplanTemplate|
	...
	# To record the result of stress test
	stress_result = {}

	is_hit_acceptable_criteria = false
	threads = start_num_threads
	testplans = []
	while((threads < target_num_threads) && !is_hit_acceptable_criteria)
		test_plan = Utilities::JMXHelper.generate_testplans(testplanTemplate, threads, increase_step, threads, warmup_period, nil, duration).first
		...
		apis.each do |api|
			...
			stress_result['parameters'] << {'API' => "#{api}", 'Average Resp. Time' => avg_response_time.round(2), 'Error Rate' => "#{ (error_rate * 100).round(2) }%"}
			puts "Error Rate for API: #{api} is #{(error_rate * 100).round(2)}% and response time is #{avg_response_time.round(2)}"
			if(error_rate > target_maximum_error_rate || avg_response_time > target_avg_response_time)
				...
				puts "#{api}: Error Rate:#{(error_rate * 100).round(2)}%; Average Response Time: #{avg_response_time.round(2)}"
				puts "==========================================================="
				#TODO, create the report for the stress testing here
				#===========================================================
				stress_result['title'] = "Maximum Threads:"
				stress_result['value'] = "#{threads}"
				is_hit_acceptable_criteria = true
			end
		end
	...
	report_link = kibana_helper.generate_stress_test_report
    ...
    kibana_helper.update_stress_result_panel stress_result
